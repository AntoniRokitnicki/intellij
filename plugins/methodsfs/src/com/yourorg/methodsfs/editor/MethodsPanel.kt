...
internal class MethodsPanel(
  ...
  fun refresh() {
    ApplicationManager.getApplication().executeOnPooledThread {
      val vf = vFile as MethodsVirtualFile
      val rows = ReadAction.compute<List<MethodRow>, RuntimeException> {
        when (val parsed = parseTarget(vf.pathInScheme())) {
      ...
      vf.update(text.toByteArray(), System.currentTimeMillis())
  ...
  private fun parseTarget(path: String): Target {
    return when {
