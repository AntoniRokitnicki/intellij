...
class GhIssuesPanel(project: Project) : GhTablePanel() {
  ...
  private fun load() {
    ...
    val endpoint = "/repos/$o/$r/issues?state=${state.text.trim()}&per_page=100"
    object : Task.Backgroundable(project, "Loading issues") {
      override fun run(indicator: ProgressIndicator) {
        ...
        val items: List<Issue> = gh.parseJson(res.out)
        SwingUtilities.invokeLater {
          items.filter { it.pull_request == null }
            .forEach { i -> addRow(i.number, i.title, i.state, i.user.login, i.html_url, i.updated_at) }
        }
      }
    }.queue()
  }
